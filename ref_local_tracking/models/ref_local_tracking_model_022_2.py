...
def ref_local_tracking_model_022_2(
    ...
    # U-Net Skips
    skip_names_main = [
        pre_trained_unet_l4_model_main.layers[19].name,
        pre_trained_unet_l4_model_main.layers[14].name,
        pre_trained_unet_l4_model_main.layers[9].name,
        pre_trained_unet_l4_model_main.layers[4].name,
    ]
    skip_names_ref = [
        pre_trained_unet_l4_model_ref.layers[19].name,
        pre_trained_unet_l4_model_ref.layers[14].name,
        pre_trained_unet_l4_model_ref.layers[9].name,
        pre_trained_unet_l4_model_ref.layers[4].name,
    ]
    ...
    up_diff1: Layer = UpSampling2D(size=(8, 8))(diff_local1)
    ...
    up_diff2: Layer = UpSampling2D(size=(4, 4))(diff_local2)
    ...
    up_diff3: Layer = UpSampling2D(size=(2, 2))(diff_local3)
    ...
    # Output
    merge1 = concatenate([diff_local4, up_diff3, up_diff2, up_diff1])
    conv1: Layer = unet_base_conv_2d(
        filter_num=25, kernel_size=1, activation="softmax"
    )(merge1)

    output: Layer = AggregationLayer(bin_size=bin_num, k_size=5, name=output_name)(
        [conv1, ref_label_input]
    )
