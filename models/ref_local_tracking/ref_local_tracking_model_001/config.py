...
ref_local_tracking_model_001_model_descriptor_default: ModelDescriptor = ModelDescriptor(
    inputs=[
        ("main_image", (256, 256, 1)),
        ("ref_image", (256, 256, 1)),
        ("bin_label", (32, 32, bin_size)),
    ],
...
def input_ref_label_preprocessing_function(label: np.ndarray) -> np.ndarray:
...
class RefModel001ModelHelper(ModelHelper):
    ...
    def get_model(self) -> Model:
        return ref_local_tracking_model_001(
            ...
            input_ref_label_name=self.model_descriptor.inputs[2][0],
            input_ref_label_shape=self.model_descriptor.inputs[2][1],
...
def single_input_main_image_preprocessing(
    ...
    resize_to = ref_local_tracking_model_001_model_descriptor_default.get_input_sizes()[
        0
    ]
...
def single_input_ref_result_preprocessing(
    img: np.ndarray, full_path_optional: Optional[Tuple[str, str]] = None
) -> np.ndarray:
    resize_to = ref_local_tracking_model_001_model_descriptor_default.get_input_sizes()[
        0
...
def single_generator(
    main_img: np.ndarray, ref_image: np.ndarray, ref_result: np.ndarray
) -> Generator:
    for index in range(1):
        main_img = np.reshape(main_img, (1,) + main_img.shape)
        ref_image = np.reshape(ref_image, (1,) + ref_image.shape)
        ref_result = np.reshape(ref_result, (1,) + ref_result.shape)
        yield [main_img, ref_image, ref_result]
