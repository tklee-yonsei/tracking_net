...
def model(
    ...
    hc_names = [
        ...
        pre_trained_unet.layers[5].name,
        pre_trained_unet.layers[2].name,
    ]
    l4_hc_model_2 = Model(
    ...
    l4_merge_2: Layer = concatenate([ref1_l4_2, main_l4_2, bin2_input], axis=3)
    l4_conv_2 = Conv2D(
        bin_num, 5, activation="relu", padding="same", kernel_initializer="he_normal",
    )(l4_merge_2)
    l4_conv_2 = Conv2D(
        bin_num,
        1,
    ...
    total_conv: Layer = Conv2D(
        bin_num, 1, activation="softmax", padding="same", kernel_initializer="he_normal"
    )(l4_conv_4)

    model: Model = Model(
        inputs=[main_input, ref1_input, bin2_input, bin3_input, bin4_input],
        outputs=[total_conv],
    )
