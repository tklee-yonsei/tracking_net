...
def tf_input_ref_label_1_preprocessing_function(label, color_info, bin_size=30):
    result = tf_image_detach_with_id_color_probability_list(
        label, color_info, bin_size, 3
    )
    result = tf.reshape(result, (256 // (2 ** 3), 256 // (2 ** 3), bin_size))
    result = tf_change_order(result, color_info[0])
    return result


def tf_input_ref_label_2_preprocessing_function(label, color_info, bin_size=30):
    result = tf_image_detach_with_id_color_probability_list(
        label, color_info, bin_size, 2
    )
    result = tf.reshape(result, (256 // (2 ** 2), 256 // (2 ** 2), bin_size))
    result = tf_change_order(result, color_info[0])
    return result


def tf_input_ref_label_3_preprocessing_function(label, color_info, bin_size=30):
    result = tf_image_detach_with_id_color_probability_list(
        label, color_info, bin_size, 1
    )
    result = tf.reshape(result, (256 // (2 ** 1), 256 // (2 ** 1), bin_size))
    result = tf_change_order(result, color_info[0])
    return result


def tf_input_ref_label_4_preprocessing_function(label, color_info, bin_size=30):
    result = tf_image_detach_with_id_color_probability_list(
